...
18. **All documentation lives in `docs/`**, the README links to it with a table. The repo root contains only README.md, CHANGELOG.md, CLAUDE.md and AI_DIARY.md — no other MD files. Conventional non-MD root artifacts (LICENSE, package.json, docker-compose.yml, …) are outside this rule's scope — it guards against documentation sprawl, not against standard tooling files.
