...
16. **Everything local only: a merge into `main` = a local git tag `vX.Y.Z`, NEVER a push.** Branches are created locally, merged into `main` locally and tagged locally — nothing is pushed. The push to the remote and the GitHub release happen exclusively on the human's explicit instruction (typically only at submission); the release note = the content of the changelog entry. The merge and the tag are done by the human.
