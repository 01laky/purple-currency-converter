...
- **Number formatting through own helpers, fixed per Figma:** `formatMoney`/`formatCount` as a custom deterministic string implementation — NOT `Intl.NumberFormat` (revised at 0.9.0: ICU's `cs-CZ` grouping separator is U+00A0/U+202F, never the plain space the design shows, and CLDR drift across runtimes changes separators) — the thousands separated by a regular space (U+0020), comma decimals, the currency code after the amount, always 2 dp. **The format does not change with the language, the browser locale, the ICU build or the runtime version** — the language changes texts, not numbers. The helpers are the only formatting place and have unit tests; the frontend never rounds values (§5).
